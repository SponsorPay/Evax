...
class Evax
  ...
  def config
    default_opts = { "compress" => true, "add_compile_date" => false }
  ...
  def build_js( group_names=[] )
    ...
    groups.each_key do |group_name|
      ...
      result_string = Evax.add_compile_date( results_string ) if config["add_compile_date"]
  ...
  def self.add_compile_date( result_string )
